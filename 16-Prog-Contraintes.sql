...
begin
    if :new.DEBUTEXP > :new.FINEXP then
        raise_application_error(-20001, 'Les dates pour l experience ne sont pas dans le bon ordre');
end if;
end;
/
...
begin
    if :new.DEBUTEXP < :new.DATECHERCHEUR then
        raise_application_error(-20001, 'Les dates ne sont pas dans le bon sens');
    end if;    
end;
/
...
if nbReac = 1 then
        raise_application_error(-20001, 'Ce nom de réactif existe déjà');
end if;
end;
/
commit;